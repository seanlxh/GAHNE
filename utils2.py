...
def load_data(dataset_str):
    ...
    if dataset_str == 'DBLP_four_area':
        ...
        label_index = ["author_label.txt"]
        train_label_index = ["all_label_train_id.txt"]
        test_label_index = ["all_label_test_id.txt"]
        ...
        node_index = ["business_dict.txt", "user_dict.txt",
                      "star_dict.txt", "reservation_dict.txt", "service_dict.txt"]
        edge_index = ["business_user.txt", "business_stars.txt",
                      "business_reservation.txt", "business_service.txt"]
        label_index = ["business_category.txt"]
        train_label_index = ["business_train_idx.txt"]
        test_label_index = ["business_test_idx.txt"]
    ...
    for index in range(len(node_index)):
        idx_reorder = parse_node_index_file("data/{}/{}".format(dataset_str, node_index[index])) #获取index列表
        all_node_num = all_node_num + len(idx_reorder)
        node_objects.append(idx_reorder)#插入node列表
    all_row = []
    all_col = []
    all_new_data = []
    all_old_data = []
...
def preprocess_tradition_adj(adj):
    """Preprocessing of adjacency matrix for simple GCN model and conversion to tuple representation."""
    adj_normalized = tradition_adj(adj + sp.eye(adj.shape[0]))
    return sparse_to_tuple(adj_normalized)



def construct_feed_dict(features, support, support_tradition, labels, labels_mask, placeholders):
    ...
    feed_dict.update({placeholders['support_tradition'][i]: support_tradition[i] for i in range(len(support_tradition))})
...
##  聚类部分
#     y_pred = DBSCAN().fit_predict(tmp_feature)
#     print(y_pred)
#     cluster_embedding = {}
#     center_embedding = {}
#     for label in y_pred:
#         if label in cluster_embedding:
#             cluster_embedding[label].append(tmp_feature[label])
#         else:
#             tmp = []
#             cluster_embedding[label] = tmp
#             cluster_embedding[label].append(tmp_feature[label])
#
#     for (k, v) in cluster_embedding.items():
#         mean_embedding = np.mean(v)
#         center_embedding[k] = mean_embedding
#
#     for idx, label in enumerate(y_pred):
#         tmp_feature[idx,:] = center_embedding[label]
#
#     print(tmp_feature)